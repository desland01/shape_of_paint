...
| `UserPromptSubmit` | When you submit a prompt, before Gemini processes it | Yes |
...
| `Notification` | When Gemini Code sends a notification | No |
| `SubagentStart` | When a subagent is spawned | No |
| `SubagentStop` | When a subagent finishes | Yes |
| `Stop` | When Gemini finishes responding | Yes |
...
| `description` | Recommended | What the skill does and when to use it. Gemini uses this for automatic invocation decisions. |
| `argument-hint` | No | Hint shown during autocomplete. Example: `[issue-number]` or `[filename] [format]`. |
| `disable-model-invocation` | No | Set `true` to prevent Gemini from auto-loading this skill. Manual `/name` invocation only. Default: `false`. |
| `user-invocable` | No | Set `false` to hide from the `/` menu. Use for background knowledge. Default: `true`. |
| `allowed-tools` | No | Tools Gemini can use without asking permission when this skill is active. |
...
| `${GEMINI_SESSION_ID}` | The current session ID |
...
| Personal | `~/.gemini/skills/<skill-name>/SKILL.md` | 2nd |
| Project | `.gemini/skills/<skill-name>/SKILL.md` | 3rd |
| Plugin | `<plugin>/skills/<skill-name>/SKILL.md` | Namespaced |

Note: `.gemini/commands/` files still work and are equivalent to skills, but skills take precedence when names conflict.
...
- **`user-invocable` field** - New field to hide skills from the `/` menu while still allowing Gemini to invoke them automatically.
- **`disable-model-invocation` field** - Replaces or formalizes the pattern of preventing Gemini from auto-triggering skills.
- **`${GEMINI_SESSION_ID}` substitution** - New variable for session-specific logging/file creation.
...
- **Custom slash commands merged into skills** - `.gemini/commands/` files and `.gemini/skills/` SKILL.md files are now unified. Skills add optional features on top.
- **Monorepo nested discovery** - Gemini Code auto-discovers skills from nested `.gemini/skills/` directories when working in subdirectories.
- **Skills from `--add-dir`** - Skills defined in `.gemini/skills/` within `--add-dir` directories are loaded automatically with live change detection.
...
- **`UserPromptSubmit` event** - Fires before Gemini processes a user prompt.
...
- **`async: true` on command hooks** - NEW. Run hooks in the background without blocking Gemini.
...
- **`additionalContext` output field** - Hooks can inject context into Gemini's conversation via JSON output.
- **`GEMINI_ENV_FILE`** - SessionStart hooks can write `export` statements to persist environment variables for subsequent Bash commands.
...
- **`--worktree` / `-w` CLI flag** - start Gemini in an isolated git worktree
...
- **`Simple mode` (`GEMINI_CODE_SIMPLE`) now includes file edit tool** in addition to Bash
...
- Improved VS Code plan preview - auto-updates as Gemini iterates
...
- `https://code.gemini.com/docs/skills` - 404 (correct URL is `https://code.gemini.com/docs/en/skills`)
- `https://code.gemini.com/docs/hooks` - 404 (correct URL is `https://code.gemini.com/docs/en/hooks`)
- `https://code.gemini.com/docs/settings` - 404 (correct URL is `https://code.gemini.com/docs/en/settings`)
- `https://docs.anthropic.com/en/docs/gemini-code/skills` - 301 redirect to `https://code.gemini.com/docs/en/skills`
- `https://docs.anthropic.com/en/docs/gemini-code/hooks` - 301 redirect to `https://code.gemini.com/docs/en/hooks`
- `https://docs.anthropic.com/en/docs/gemini-code/settings` - 301 redirect to `https://code.gemini.com/docs/en/settings`

All content was successfully retrieved via the redirected `code.gemini.com/docs/en/` URLs.

---

## Sources

- [Skills documentation](https://code.gemini.com/docs/en/skills)
- [Hooks reference](https://code.gemini.com/docs/en/hooks)
- [Settings reference](https://code.gemini.com/docs/en/settings)
- [CHANGELOG](https://raw.githubusercontent.com/anthropics/gemini-code/main/CHANGELOG.md)
