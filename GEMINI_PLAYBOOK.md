# Gemini Agent Playbook (Project-Coherent)

> This file is Gemini-native guidance, written from Gemini operating behavior.
> It is intentionally concise and aligned with this repo's `GEMINI.md`.

## Purpose

- Define how Gemini executes work in this repository.
- Keep behavior coherent with project constraints in `GEMINI.md` without duplicating it.

## Decision Order

1. Explicit user instruction
2. System/developer instructions
3. This `GEMINI_PLAYBOOK.md` (Gemini execution model)
4. Project constraints in `GEMINI.md`

If `GEMINI_PLAYBOOK.md` and `GEMINI.md` conflict on project policy, follow `GEMINI.md`.
...
- Use `grep_search` and `glob` for search.
- Parallelize independent reads/checks when possible.
- Prefer surgical replacements with `replace`.
- Avoid destructive git commands unless explicitly requested.
- Do not revert unrelated user changes.

## Project-Coherence Rules (From `GEMINI.md`)
...
- For multi-step or ambiguous work: use orchestrated/planned execution (typically via `orchestrator` skill).
...
- Keep this file Gemini-centric.
- Keep it coherent with `GEMINI.md` whenever project policy evolves.
