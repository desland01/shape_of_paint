...
### expo-app-audit (legacy: `/Users/thebeast/.claude/skills/expo-app-audit.md`)
- **Missing: `name` field in YAML frontmatter** — the frontmatter block appears AFTER the H1 heading, not at the top of the file. The file opens with `# Expo App Production Readiness Audit` on line 1, then the YAML block on lines 3-7. YAML frontmatter must be the very first thing in the file (starting at byte 0) for parsers to recognise it. As written, the frontmatter is invalid and Claude Code will not parse it.
- **Wrong format: frontmatter not at top of file** — full content treated as body text, not metadata.
- **Also: legacy single-file format** — a directory-format version exists at `~/.claude/skills/expo-app-audit/SKILL.md` (which is correctly formatted). The legacy `.md` file is a duplicate and should be removed.
- **Action:** Delete `/Users/thebeast/.claude/skills/expo-app-audit.md`; the directory version is correct.

### react-native-accessibility (legacy: `/Users/thebeast/.claude/skills/react-native-accessibility.md`)
...
- **Also: legacy single-file format** — directory version exists at `~/.claude/skills/react-native-accessibility/SKILL.md` (correctly formatted). The legacy file is a duplicate.
- **Action:** Delete `/Users/thebeast/.claude/skills/react-native-accessibility.md`.

### expo-audit-remediation (legacy: `/Users/thebeast/.claude/skills/expo-audit-remediation.md`)
- **Missing: `name` field** — frontmatter appears AFTER the H1 heading. Same broken pattern; frontmatter not at top.
- **Also: legacy single-file format** — directory version exists at `~/.claude/skills/expo-audit-remediation/SKILL.md` (correctly formatted). The legacy file is a duplicate.
- **Action:** Delete `/Users/thebeast/.claude/skills/expo-audit-remediation.md`.

### frontend-design-auto (legacy: `/Users/thebeast/.claude/skills/frontend-design-auto.md`)
...
- **Also: legacy single-file format** — directory version exists at `~/.claude/skills/frontend-design-auto/SKILL.md` (which includes a `name` field and is correct). The legacy file is a duplicate.
- **Action:** Delete `/Users/thebeast/.claude/skills/frontend-design-auto.md`.

### trpc-type-safety (`/Users/thebeast/.claude/skills/trpc-type-safety/SKILL.md`)
...
- **Action:** Remove the second YAML block from the body. The content in it (section definitions) is not loaded by Claude Code anyway and adds bloat.

### qa.backup.20260210134230 (`/Users/thebeast/.claude/skills/qa.backup.20260210134230/SKILL.md`)
- **Invalid `name` field** — `name: qa` (only 2 characters, fine for length). However, the skill path contains `.backup.20260210134230` which is neither a clean skill name nor a valid directory name for a skill. This directory format (`qa.backup.*`) will confuse skill discovery — the system expects `~/.claude/skills/<skill-name>/SKILL.md` where the directory name is the skill name. This skill's directory name does not match its `name` field (`qa`) and the dot-containing directory name is unusual.
- **Action:** This is clearly a backup artifact. If the canonical `qa` skill is needed, it should live at `~/.claude/skills/qa/SKILL.md`. The backup directory should either be deleted or moved outside `~/.claude/skills/`.

### skill-creator (`/Users/thebeast/.claude/skills/skill-creator/SKILL.md`)
- **Wrong platform target** — The skill references `$CODEX_HOME/skills` (OpenAI Codex) and paths like `~/.codex/skills`. This skill was authored for OpenAI Codex, not Claude Code. It will not function correctly as a Claude Code skill. The `description` says "Codex" explicitly ("extends Codex's capabilities"). The `name` and `description` fields are present and valid per format, but the content is platform-mismatched.
- **Action:** Either rewrite for Claude Code (replacing Codex references with Claude Code references and correct paths) or remove it. Loading it will give incorrect guidance.

### skill-installer (`/Users/thebeast/.claude/skills/skill-installer/SKILL.md`)
...
### gtm-conversion-tracking (`/Users/thebeast/.claude/skills/gtm-conversion-tracking/SKILL.md`)
- **Unknown frontmatter field: `triggers`** — The `triggers` field is not part of the Anthropic SKILL.md spec. It will be silently ignored. The trigger information should be moved into the `description` field where Claude Code can actually use it for auto-invocation.
- **Action:** Move trigger phrases from `triggers` into the `description` field. Remove `triggers`.

### expo-app-audit (`/Users/thebeast/.claude/skills/expo-app-audit/SKILL.md`)
...
### react-native-accessibility (`/Users/thebeast/.claude/skills/react-native-accessibility/SKILL.md`)
- **Unknown frontmatter field: `when_to_use`** — Same issue as expo-app-audit. Not a recognised spec field.
- **Action:** Merge `when_to_use` content into `description`. Remove `when_to_use`.

### expo-audit-remediation (`/Users/thebeast/.claude/skills/expo-audit-remediation/SKILL.md`)
...
### building-native-ui (`/Users/thebeast/.claude/skills/building-native-ui/SKILL.md`)
- **Unknown frontmatter fields: `version`, `license`** — Not spec fields. Silently ignored.
- **Action:** Remove `version` and `license`.

### bullmq-specialist (`/Users/thebeast/.claude/skills/bullmq-specialist/SKILL.md`)
...
### twilio-communications (`/Users/thebeast/.claude/skills/twilio-communications/SKILL.md`)
- **Unknown frontmatter field: `source`** — Same issue as bullmq-specialist.
- **Action:** Remove `source` or move to a body comment.

### clerk-nextjs-patterns (`/Users/thebeast/.claude/skills/clerk-nextjs-patterns/SKILL.md`)
...
### fastify-best-practices (`/Users/thebeast/.claude/skills/fastify-best-practices/SKILL.md`)
- **Unknown frontmatter field: `metadata`** — `metadata.tags` is not a spec field.
- **Action:** Remove `metadata` block.

### opentelemetry (`/Users/thebeast/.claude/skills/opentelemetry/SKILL.md`)
...
### product-manager (`/Users/thebeast/.claude/skills/product-manager/SKILL.md`)
- **Unknown frontmatter fields: `version`, `author`, `category`, `tags`** — None are spec fields.
- **Action:** Remove all four non-spec fields.

### remotion-best-practices (`/Users/thebeast/.claude/skills/remotion-best-practices/SKILL.md`)
- **Unknown frontmatter field: `metadata`** — `metadata.tags` is not a spec field.
- **Action:** Remove `metadata` block.

### prisma-database-setup (`/Users/thebeast/.claude/skills/prisma-database-setup/SKILL.md`)
- **Unknown frontmatter fields: `license`, `metadata`** — Not spec fields.
- **Action:** Remove `license` and `metadata` block.

### design-reference (`/Users/thebeast/.claude/skills/design-reference/SKILL.md`)
- **Unknown frontmatter fields: `license`, `metadata`** — `metadata.author` and `metadata.version` are not spec fields.
- **Action:** Remove `license` and `metadata` block.

### frontend-react-design (`/Users/thebeast/.claude/skills/frontend-react-design/SKILL.md`)
- **Unknown frontmatter fields: `autoApply`, `globs`, `keywords`** — These are Cursor-specific frontmatter fields, not Claude Code spec fields. The `globs` field in Cursor controls which file contexts activate a skill; in Claude Code this is done via the `description` field. The `autoApply` and `keywords` fields are not spec fields.
- **Action:** Remove `autoApply`, `globs`, and `keywords`. Move any trigger keywords into the `description` field.

### frontend-trades-design (`/Users/thebeast/.claude/skills/frontend-trades-design/SKILL.md`)
...
### mobile-cro-optimizer (`/Users/thebeast/.claude/skills/mobile-cro-optimizer/SKILL.md`)
- **Unknown frontmatter fields: `autoApply`, `globs`, `keywords`** — Same Cursor-specific fields.
- **Action:** Remove `autoApply`, `globs`, and `keywords`. Move trigger keywords into `description`.

### sterling-sky-local-seo (project: `/Users/thebeast/Paint-For-Money/shape-of-paint/.claude/skills/sterling-sky-local-seo/SKILL.md`)
...
### crash (`/Users/thebeast/.claude/skills/crash/SKILL.md`)
- **Project-hardcoded path** — Line 46: `pnpm type-check 2>&1 | tail -5` path is hardcoded to `/Users/thebeast/sales-console/Sales-console`. This skill is in the global `~/.claude/skills/` directory and will fail for any project that is not Sales Console.
- **Suggestion:** Replace the hardcoded path with a shell-injection using `$CLAUDE_PROJECT_DIR` or make the path dynamic.

### deploy (`/Users/thebeast/.claude/skills/deploy/SKILL.md`)
...
### status (`/Users/thebeast/.claude/skills/status/SKILL.md`)
...
### verify (`/Users/thebeast/.claude/skills/verify/SKILL.md`)
- **Project-specific content in global skill** — References "Sales Coach project", `pnpm type-check`, `pnpm build`.
- **Suggestion:** Move to project level or make configurable.

### visual-eval (`/Users/thebeast/.claude/skills/visual-eval/SKILL.md`)
...
### new-router (`/Users/thebeast/.claude/skills/new-router/SKILL.md`)
...
### new-widget (`/Users/thebeast/.claude/skills/new-widget/SKILL.md`)
- **Missing `allowed-tools`** — Creates files but has no `allowed-tools` field.
- **Suggestion:** Add `allowed-tools: Read, Write, Edit`.

### create-command (`/Users/thebeast/.claude/skills/create-command/SKILL.md`)
- **Cursor-specific path in body** — Step 4 instructs creating a folder at `.cursor/skills/commands/[command-name]/` and Step 5 mentions "Cursor's skill list". This is a Cursor-specific skill that has been copied into the Claude Code skills directory. The paths and editor references are wrong for Claude Code.
- **Suggestion:** Update paths to `.claude/skills/[command-name]/` and references to Claude Code.

### create-rule (`/Users/thebeast/.claude/skills/create-rule/SKILL.md`)
- **Cursor-specific content** — The entire body references `.cursor/rules/`, `.mdc` files, and "Cursor rules". This is a Cursor-specific skill in the Claude Code directory. In Claude Code, rules live in `.claude/rules/*.md` (plain `.md`, not `.mdc`).
- **Suggestion:** Update for Claude Code or label clearly as Cursor-only.

### create-subagent (`/Users/thebeast/.claude/skills/create-subagent/SKILL.md`)
- **Cursor-specific content** — References `.cursor/agents/`, `~/.cursor/agents/`, and "Cursor" throughout. This is a Cursor-specific skill. Claude Code uses a different agent model.
- **Suggestion:** Update for Claude Code or label clearly as Cursor-only.

### migrate-to-skills (`/Users/thebeast/.claude/skills/migrate-to-skills/SKILL.md`)
- **Cursor-specific content** — References `.cursor/rules/*.mdc`, `.cursor/commands/*.md`, `.cursor/skills/`. In Claude Code, the paths are `.claude/rules/*.md` and `.claude/skills/`. The migration tool would produce incorrect output paths if run under Claude Code.
- **Suggestion:** Update source/destination paths for Claude Code conventions.

### sync-skills-to-claude (`/Users/thebeast/.claude/skills/sync-skills-to-claude/SKILL.md`)
- **References Cursor directories** — Copies from `~/.cursor/skills/` and `~/.cursor/skills-cursor/`. This skill exists to bridge Cursor→Claude, which is valid, but the description should clarify this is a one-way sync from Cursor.
- **No issues with format** — name, description are valid. Just a usage note.

### skill-creator + skill-installer (critical above)
- Already covered in Critical Issues — platform mismatch (Codex vs Claude Code).

### local-seo (`/Users/thebeast/.claude/skills/local-seo/SKILL.md`)
...
- The legacy file at `/Users/thebeast/.claude/skills/google-ads-account-diagnosis.md` is an exact duplicate of the directory version. Both have valid frontmatter with `name` and `description`.
...
**Note:** Skills with only unknown/non-spec frontmatter fields (Warnings) are otherwise clean — they have valid `name` and `description` and will function correctly in Claude Code. The unknown fields are silently ignored.
